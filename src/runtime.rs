...
pub fn invoke(id: String, mut params: Vec<String>) -> Result<String, RuntimeError> {
    ...
    */
    ...
    let val: String = con_pub.get(&id.replace("\"","")).unwrap_or("Function not created or published".to_string());
    ...
    let output = Command::new(val).args(&v7).output().expect("Error");

    let response = std::str::from_utf8(&output.stdout).unwrap();

    //return the result
    Ok(response.to_string().split("FaaSRESULT:").collect::<Vec<&str>>()[1].to_string())
}
